...
function order_fee($order, $goods, $consignee = NULL)
{
    ...
    if (isset($consignee) && $order['shipping_id'] > 0 && $total['real_goods_count'] > 0)
    {
        ...
        if (!empty($shipping_info))
        {
            ...
            $sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('cart') . " WHERE " . get_cart_cond() . " AND `extension_code` != 'package_buy' AND `is_shipping` = 0";
    ...
    if ($order['extension_code'] == 'exchange_goods')
    {
        $sql = 'SELECT SUM(eg.exchange_integral) '.
               ...
               "WHERE c.goods_id = eg.goods_id AND " . get_cart_cond('c.') .
...
function cart_goods_exists($id, $spec, $type = CART_GENERAL_GOODS)
{
    ...
    $sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('cart').
            "WHERE " . get_cart_cond() . " AND goods_id = '$id' ".
...
function cart_weight_price($type = CART_GENERAL_GOODS)
{
    ...
    /* 计算超值礼包内商品的相关配送参数 */
    $sql = 'SELECT goods_id, goods_number, goods_price FROM ' . $GLOBALS['ecs']->table('cart') . " WHERE extension_code = 'package_buy' AND " . get_cart_cond();
...
function addto_cart_combo($goods_id, $num = 1, $spec = array(), $parent = 0, $group = 0)
{
    ...
    if ($parent > 0)
    {
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('cart_combo') .
                " WHERE goods_id='$parent' AND " . get_cart_cond() . " AND extension_code <> 'package_buy' AND group_id = '$group'";
        if ($GLOBALS['db']->getOne($sql) == 0)
        {
            $GLOBALS['err']->add($GLOBALS['_LANG']['no_basic_goods'], ERR_NO_BASIC_GOODS);
...
//        $num -= $parent['goods_number'];
    ...
    /* 把赠品删除 */
    $sql = "DELETE FROM " . $GLOBALS['ecs']->table('cart_combo') . " WHERE " . get_cart_cond() . " AND is_gift <> 0";
    $GLOBALS['db']->query($sql);
...
function clear_cart($type = CART_GENERAL_GOODS)
{
    $sql = "DELETE FROM " . $GLOBALS['ecs']->table('cart') .
            " WHERE " . get_cart_cond() . " AND rec_type = '$type'";
    $GLOBALS['db']->query($sql);
}
...
function user_bonus($user_id, $goods_amount = 0)
{
    ...
    $sql = "SELECT t.type_id, t.type_name, b.amount, b.bonus_id " .
...
function exist_real_goods($order_id = 0, $flow_type = CART_GENERAL_GOODS)
{
    ...
    {
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('cart') .
                " WHERE " . get_cart_cond() . " AND is_real = 1 " .
...
function flow_order_info()
{
    ...
    if (!isset($order['bonus_id']))
    {
    	$order['bonus_id'] = 0; // 初始化红包
        $order['bonus'] = 0;    
    }
...
//    }
    ...
    if ($num > 0)
    {
         /* 检查该商品是否已经存在在购物车中 */
        $sql = "SELECT goods_number FROM " .$GLOBALS['ecs']->table('cart').
                " WHERE " .get_cart_cond(). " AND goods_id = '" . $package_id . "' ".
